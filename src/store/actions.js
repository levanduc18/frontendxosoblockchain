import { isInstallMetaMask, getBalance, getNetwork } from "@/utils/getWeb3";
import {
  getBasicLottoInfo,
  getCurrentLottoInfo,
} from "@/utils/getLotteryContract";
import { getUSDTBalance, getAllowanceBalance } from "@/utils/getUSDTContract";
import "@/utils/getTicketContract";
import { getUserTicketsAllRound } from "@/utils/getTicketContract";

export default {
  async checkInstallMetaMask({ commit }) {
    const isInstall = await isInstallMetaMask();
    commit("setIsInstallMetaMask", isInstall);
  },
  ...
  async setUSDTAllowanceBalance({ commit, state }) {
    const { account } = state;
    const USDTAllowanceBalance = await getAllowanceBalance(account);
    commit("setUSDTAllowanceBalance", USDTAllowanceBalance);
  },
  ...
  setIsOpenSettingModal({ commit }, payload) {
    commit("setIsOpenSettingModal", payload);
  },
};
